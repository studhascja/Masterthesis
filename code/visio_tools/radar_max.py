...
# Dict: (standard, freq, bw, qos, proto) -> Maximal-Latenz
latency_data = {}
    ...
    for file in files:
        if file == "latencys_0":
            ...
            if rtts:
                ...
                latency_data[key] = np.max(rtts)  # <-- Hier verwenden wir Maximum statt Mittelwert
...
# Berechnen der durchschnittlichen Differenzen (jetzt basierend auf Maximal-Latenz)
results = [
...
radar_chart(factors, results, f"Einflussfaktoren auf maximale Latenz (Config {config})")

print("\nDurchschnittliche Differenzen pro Faktor (basierend auf Maximum):")
