...
\usepackage{lmodern} % or another scalable font package
\usepackage{pifont}
...
\usepackage{xcolor}
\definecolor{gold}{rgb}{1,0.84,0} % Define gold color
\definecolor{typblue}{HTML}{1F3A93}
\definecolor{funblue}{HTML}{1B77CF}
\definecolor{kwgreen}{HTML}{1F7A1F}
\definecolor{structmag}{HTML}{C2185B}
...
\newcommand{\circnum}[2][fill=yellow!20,draw=orange,thick,text=black]{%
  \tikz[baseline=(char.base)]{
    \node[shape=circle,inner sep=1pt,#1] (char) {\textbf{#2}};
  }%
}
...
\include{chapter_test_environment}
...
\bibliographystyle{IEEEtran}  % this is intended to be used together with German language settings; replace if you desire a different style
\bibliography{thesis}
\end{document}
